...
def get_results_csv_path(
    ...
    if result_tag in {"exact", "heuristic"}:
        ...
        if algo in ["BST", "IC"]:
            algo_type = "exact"
        elif algo in ["MA", "KMA", "DKMA", "GNN-KMA", "GNN-KMA-2", "GNN-DKMA"]:
            algo_type = "heuristic"
        else:
            algo_type = "unknown"
...
def _undirected_worker_run(
    ...
    try:
        ...
        if algo == "MA":
            ...
            from run_hybrid import dkma_solve_undirected
            fvs = dkma_solve_undirected(
                n,
                edges,
                pop_size,
                max_gens,
            ...
            from run_hybrid import gnn_dkma_solve_undirected
            fvs = gnn_dkma_solve_undirected(
                n,
                edges,
                gnn_version="v1",
                gnn_threshold=gnn_threshold,
...
def run_algorithm_with_timeout(
    ...
    if algo in {"MA", "KMA", "GNN-KMA", "GNN-KMA-2", "DKMA", "GNN-DKMA"}:
        try:
...
def run_algorithm(
    ...
    if algo == "MA":
        ...
        from run_hybrid import dkma_solve_undirected
        fvs, stage_metrics = dkma_solve_undirected(
            n,
            edges,
            pop_size=pop_size,
            max_gens=max_gens,
...
def run_on_file(
    ...
    if algo == "ALL":
        algos_to_run = ["BST", "IC", "MA", "KMA", "DKMA", "GNN-KMA", "GNN-KMA-2", "GNN-DKMA"]
    elif algo == "PUREALGO":
...
def main():
    ...
    parser.add_argument(
        "--algo", required=True,
        choices=["BST", "IC", "MA", "KMA", "DKMA", "GNN-KMA", "GNN-KMA-2", "GNN-DKMA", "ALL", "PUREALGO"],
        help="Algorithm to run: BST (exact), IC (exact), MA/KMA/DKMA (heuristic), GNN-KMA/GNN-KMA-2/GNN-DKMA (hybrid), ALL (compare), PUREALGO (BST+IC+MA+KMA+DKMA)"
    )
    parser.add_argument(
        "--test", required=True,
    ...
    if len(all_results) > 1 or args.quiet:
        ...
        if args.algo == "ALL":
            algos_ran = ["BST", "IC", "MA", "KMA", "DKMA", "GNN-KMA", "GNN-KMA-2", "GNN-DKMA"]
