...
\begin{frame}{Iterative Compression for FVS}
	\textbf{Idea:} Build solution incrementally and compress when it gets too large.

	\medskip
	\textbf{Algorithm}
	\begin{itemize}
		\item Order vertices $v_1,\dots,v_n$
		\item Initialize $F \leftarrow \{v_1\}$
		\item For $i = 2$ to $n$:
		      \begin{itemize}
			      \item $F \leftarrow F \cup \{v_i\}$
			      \item If $|F| = k+1$:
			            \begin{itemize}
				            \item Try all partitions $(F_1,F_2)$ of $F$
				            \item Find $Y \subseteq V \setminus F_2$, $|Y|\le k-|F_1|$
				            \item If $F_1 \cup Y$ is an FVS, update $F$
				            \item Else return \textbf{NO}
			            \end{itemize}
		      \end{itemize}
		\item Return $F$ if $|F|\le k$
	\end{itemize}

	\medskip
	\textbf{Key Insight:} Compress size $k\!+\!1$ solution to size $k$ \\
...
\begin{frame}{Kernelization + Bounded Search Tree}
	...
	\textbf{Algorithm}
	\begin{itemize}
		\item Apply reduction rules:
		      \begin{itemize}
			      \item Remove degree $\le 1$ vertices
			      \item Contract degree-2 vertices
			      \item Include self-loop vertices
		      \end{itemize}
		\item Obtain kernel $(G',k')$
		\item Recursive search:
		      \begin{itemize}
			      \item If $G'$ acyclic: return solution
			      \item If $k'<0$: return \textbf{NO}
			      \item Find a cycle $C$
			      \item For each $v \in C$:
			            \begin{itemize}
				            \item Recurse on $(G'-v,\, k'-1)$
			            \end{itemize}
		      \end{itemize}
	\end{itemize}
...
\begin{frame}{Memetic Algorithm for FVS}
	...
	\textbf{Algorithm}
	\begin{itemize}
		\item Encode solution as a permutation of vertices
		\item Decode by removing vertices until graph is acyclic
		\item Initialize population (greedy + random)
		\item For each generation:
		      \begin{itemize}
			      \item Evaluate fitness: $-|FVS|$
			      \item Select parents (tournament)
			      \item Crossover (cycle-aware)
			      \item Mutation (swap cycle-heavy vertices)
			      \item Local search (hill-climbing)
		      \end{itemize}
		\item Return best solution found
	\end{itemize}
